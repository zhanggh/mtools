...
public class RandomValidateCode {
    ...
    public void getRandcode(HttpServletRequest request,
        ...
        session.removeAttribute(CoreConstans.RANDOMCODEKEY);
        session.setAttribute(CoreConstans.RANDOMCODEKEY, randomString);
        System.out.println(randomString);
        g.dispose();
        try {
            ImageIO.write(image, "jpg", response.getOutputStream());//将内存中的图片通过流动形式输出到客户端
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
